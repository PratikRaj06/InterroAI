...
const TestSetup = () => {
    ...
    return (
        <div className='w-screen h-full flex flex-col items-center bg-blue-light'>
            <Header />
            <div className={` ${isPortrait ? 'md:w-10/12 w-11/12' : 'lg:w-1/2 w-2/3'} bg-white flex flex-col items-center my-5 py-10 rounded-lg shadow-lg`}>
                <h1 className='lg:text-4xl md:text-3xl text-2xl text-center font-bold pb-5 px-5 text-blue-medium'>System Compatibility Check</h1>
                <div className='w-10/12 border-[1px] border-blue-dark border-opacity-25 mt-5'></div>
                <div className='w-10/12 flex flex-col items-center bg-white rounded-lg p-5'>
                    ...
                    <div className='w-11/12 rounded-lg relative'>
                        {warn && (
                            <p className='text-center min-w-max absolute top-3 left-1/2 transform -translate-x-1/2 bg-opacity-80 backdrop-blur-lg bg-red text-white px-5 py-1 rounded-lg font-regular lg:text-xl text-lg z-10'>
                                NO FACE DETECTED
                            </p>
                        )}
                ...
                <div className='w-10/12 flex flex-col items-center my-5 bg-white rounded-lg p-5'>
                    ...
                    <div className='w-full my-5'>
                        ...
                        <p className='border-[1px] border-blue-regular border-opacity-50 rounded-md min-h-8 p-1 my-3'>{transcript}</p>
                    </div>
                    <button onClick={listening ? SpeechRecognition.stopListening : startSpeaking} className='bg-blue-regular px-10 text-white py-2 rounded-lg'>
                ...
                <div className='w-10/12 flex flex-col items-center my-5 bg-white rounded-lg p-5'>
                    <div className='w-full mb-5 flex items-center justify-between'>
                        ...
                        <button onClick={() => setMute(!mute)} className='bg-blue-regular text-2xl rounded-full text-white p-2 flex items-center justify-center'>
                ...
                <label htmlFor='check' className='w-10/12 flex items-center justify-center gap-3 cursor-pointer mt-10'>
                    <input id='check' onChange={() => setAccepted(prev => !prev)} type="checkbox" className='accent-blue-regular h-4 w-4' />
                    <p>I accept that all components are operating properly</p>
                </label>

                <button onClick={() => { navigate('/home/interview-room') }} disabled={!accepted} className='w-10/12 bg-blue-medium text-white py-2 my-5 rounded-lg text-center text-xl font-semibold'>Enter Virtual Room</button>
            </div>
        </div>
    );
};
